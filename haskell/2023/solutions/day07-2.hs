...
handStrength hand = let jokers = length $ filter (=='J') hand
                        noJokers = reverse $ sort $ map length (group $ sort $ filter (/='J') hand)
                        handType = if jokers == 5 then [5] else (jokers + head noJokers) : tail noJokers
                    in case handType of
                       [5]             -> 8
                       [4, 1]          -> 7
                       [3, 2]          -> 6
